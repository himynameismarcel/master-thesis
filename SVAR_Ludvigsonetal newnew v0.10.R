        ...
        # other constraints:
          ...
          # non-negative constraints
            ...
            #------------------------
                  ...
                  # according to LMN(2018):
                  # (1) the first inequality is alrge when the financial
                  # uncertainty shock in 1987 is large:
                  k1.maxG <-  as.vector(epsilon_t %>% 
                                       filter(yearmon == "Oct 1987") %>%
                                       dplyr::select(financial_h1))
            
                  # (2) the second inequality is large for the most extreme
                  # financial or macro uncertainty shock in September 2008:
                  k2.maxG <- as.vector(sum(epsilon_t %>% 
                                       filter(yearmon == "Sep 2008") %>%
                                       dplyr::select(financial_h1),
                                       epsilon_t %>% 
                                       filter(yearmon == "Sep 2008") %>%
                                       dplyr::select(macro_h1))
                                       )
                  
                  # (3) the third inequality is large for the most extreme
                  # macro uncertainty shock in December 1970:
                  
                  k3.maxG <- as.vector(
                                  (epsilon_t %>% 
                                     filter(yearmon == "Dec 1970") %>%
                                      dplyr::select(macro_h1))
                  )      
                  
                  # (4) the fourth inequality is largest when the real activity
                  # shocks found during the great recession are cumulatively
                  # most negative:
                  k4.maxG <- as.vector(sum(epsilon_t %>% filter(
                                      yearmon >= "Dec 2007" &
                                        yearmon <= "Jun 2009") %>%
                                      dplyr::select(lip)))  
                  
                  # (5) the fifth inequality is largest for the most
                  # extreme cumulationof financial and macro uncertainty
                  # in October 1979:
                  k5.maxG <- as.vector(sum(
                            (epsilon_t %>% filter(yearmon == "Oct 1979") %>%
                               dplyr::select(financial_h1)),             
                              (epsilon_t %>% filter(yearmon == "Oct 1979") %>%
                                 dplyr::select(macro_h1))                   
                  ...
                  # (6) the sixth inequality is largest for when the macro
                  # uncertainty shocks in 2011:07 - 2011:08 are cumulatively
                  # largest:
                  k6.maxG <- as.vector(sum(
                              sum(epsilon_t %>%
                                    filter(yearmon >= "Jul 2011" & 
                                    yearmon <= "Aug 2011") %>%
                          dplyr::select(financial_h1)),
                              sum(epsilon_t %>% 
                                    filter(yearmon >= "Jul 2011" & 
                  ...
                  # now we only still have to add the correlation constraints:
                  k7.maxG <- as.vector(abs(c_M.S1))
                  k8.maxG <- as.vector(abs(c_F.S1))
                  k9.maxG <- as.vector(abs(c_M.S2))
                  k10.maxG <- as.vector(abs(c_F.S2))
                  
                  # we bind together all values associated with maxG.event
                  maxG.all <- as.vector(unlist(c(k1.maxG, k2.maxG,
                                                   k3.maxG, k4.maxG,
                                                   k5.maxG, k6.maxG,
                                                   k7.maxG, k8.maxG,
                                                   k9.maxG, k10.maxG)))
                  
                  # next, we calculate the associated value for maxGevent:
...
#######################
    ...
    #---------------------------------------------------------------------------
    # 28.06.2020 (Marcel)
    # Note, the function 'extract.coefs.irf_maxG' was moved to the R-Script 
    # 20200628_functions.R  
    #--------------------------------------------------------------------------- 
    ...
    impulse.responses_all.SVAR <- 
      ...
      geom_line(data=plot_SVAR.irfs.maxG.CONSTR_ALL, aes(x=step, y=series_value),
                colour="black", size=1, linetype=5) +
      labs(color=NULL) + 
...
#######################
### Figure: Time series of shocks from maxG-solution
#######################
    ...
    time.series_epsilon_t_maxG <- ggplot(data = epsilon_t_maxG.tidy) +
      ...
      # 12.07.2020:
      # in order to know where the respective series have the largest
      # shocks, we have to perform the below analyses:
      epsilon_t_maxG %>% arrange(lip)
      epsilon_t_maxG %>% arrange(desc(lip))
...
## the FEVD for the Baseline Model.
## This step is actually not necessary because large parts would just be 
## a repeition of the above algorithm to arrive at the identified set and then 
## further generate the Psis and the Thetas, below we source the the script
## 'SVAR_Ludvigson_incl_FEVD_v0.5.R'.
## Hence, before executing the script 'SVAR_Ludvigson_incl_FEVD_v0.5.R' make
## sure that this script was executed because the script 
## 'SVAR_Ludvigson_incl_FEVD_v0.5.R' needs the identified set as a starting 
## point. The only difference to the above is that the script 
## 'SVAR_Ludvigson_incl_FEVD_v0.5.R' starts 

if(!exists("foo", mode="function")) source("SVAR_Ludvigsonetal_incl_FEVD_v0.5.R")      
