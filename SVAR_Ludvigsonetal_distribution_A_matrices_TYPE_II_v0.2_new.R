...
###-------------------------------
    ...
    k1 <- 4.2 # 2.8 std. dev. above mean in 1987:10 for e_Ft
    k2 <- 4.3 # 4.6 std. dev. above mean in 2008:09 for e_Ft
    k3 <- 4.2 # 4.7 std. dev. above mean in 2008:09 for e_Mt
    k4 <- 4.0   # 4 std. dev. above mean in 1970:12 for e_Mt 
...
A_0_valid.ALL_CONSTR.II.YM <- do.call(rbind, 
                                  lapply(A_0_valid.ALL_CONSTR.II, function(x) x[2,1]))
colnames(A_0_valid.ALL_CONSTR.II.YM) <- "YM"

A_0_valid.ALL_CONSTR.II.YF <- do.call(rbind, 
                                  lapply(A_0_valid.ALL_CONSTR.II, function(x) x[2,3]))
colnames(A_0_valid.ALL_CONSTR.II.YF) <- "YF"
# lastly, we create a new matrix where we add these two column vectors together:
A_0_valid.ALL_CONSTR.II.comb <- as_tibble(cbind(A_0_valid.ALL_CONSTR.II.YM, 
                                             A_0_valid.ALL_CONSTR.II.YF))
...
###--------------------------------------------------------------------------------------------
    ...
    A_0_valid.ALL_CONSTR.II.tidy <- A_0_valid.ALL_CONSTR.II.comb   %>%
                            gather(series, data)
    # and ultimately we add another column called NO_CONSTR
    # and muliply the data-series with 100:
    A_0_valid.ALL_CONSTR.II.tidy <- A_0_valid.ALL_CONSTR.II.tidy %>%
    ...
    ggplot(data=A_0_valid.NO_CONSTR.tidy) + 
      geom_histogram(aes(x=data, y=(..count..)/sum(..count..),
                     ...
                     alpha = 0.8,
                     binwidth=0.1) + 
      geom_histogram(data=A_0_valid.ALL_CONSTR.II.tidy,
