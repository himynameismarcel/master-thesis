...
# Achtung: macroUncertainty_index kommt aus dem Skript 12052018_thesis_master.R!
# therefore we have to run the following parts out of the script 12052018_thesis_master.R
# which we have copied in here for convenience:

            #########################################################################
            ## See lines 1709 - 1742 in 12052018_thesis_master.R:
            ###############################
...
# Achtung: additional_vars kommt aus dem Skript 23042018_thesis_master.R!
# therefore we have to run the following parts out of the script 23042018_thesis_master.R
# which we have copied in here for convenience:


            #########################################################################
            ## See lines 979 - 1021 in 23042018_thesis_master.R:
            ###############################
            ...
            # additional_vars <- read.csv(file="additional_vars.csv", 
            #                             header=TRUE, sep=",",
            ...
            # next, we create the three variable 'month', 'year' and 'day' using
            # the 'separate()' - function from the 'tidyr' - package
            additional_vars <- separate(additional_vars, "date", 
                                        c("year", "month", "day"), sep = "-", 
                                        remove=FALSE, convert=TRUE)
            ...
            additional_vars <- as.data.frame(additional_vars %>%
                              dplyr::select(-c(DATE.1, Date, Open, High, 
                                               Low, Adj.Close, Volume)))
            #########################################################################


SVAR.data <- as_tibble(right_join(x = financialUncertainty_index[, 
                                      c("h1","my")],
                                        y = SVAR.data,
                                        by = "my") %>%
...
# Marcel, 03.03.2020: Note that we have limited the data up until
# 2018-03-01 because for the correlation constraints we use the 
# volatility-data which we only have available until 2018/02 as well!
# Update: SVAR.data anway only contains data up until 2017/12, 
# so we can declare for our visibility 2017/12 to be the upper
# bound (or just leave it away);
SVAR.data.sub <- SVAR.data  %>%
  filter(Date <="2017-12-01" & Date >= "1962-07-01")

# nachdem wir oben filtern und den neuen Datensatz als 
# SVAR.data.sub anlegen, müssen wir den unten stehenden
# Befehl entsprechend anpassen:
# SVAR.data.sub <- SVAR.data %>%
#               dplyr::select(macro_h1, ip, financial_h1)
SVAR.data.sub <- SVAR.data.sub %>%
              dplyr::select(macro_h1, ip, financial_h1)

## Marcel, 03.03.2020
## Preparation and Calculation of returns-data from SP500:
          sp500 <- as.data.frame(additional_vars$sp500)
          #  the data in additional_vars (and consequently also the data
          #  for the sp500 time-series) runs from 1962/07 until 2018/02;
          #  this means that we have 
          dim(sp500) # --> 668 index-entries;
          colnames(sp500) <- c("sp500_prices")
          # then we store the returns in a dedicated time-series object
          sp500_returns <- as.data.frame(diff(log(sp500$sp500_prices)))
          colnames(sp500_returns) <- c("sp500_returns")
          # this time-series object consequently only has 667 entries
          # (as compared to the original series with 668 entries),
          # because by calculating the returns, we lose one entry
          # through differencing the logs; this means that the
          # return-series starts in 1962/08, instead of 1962/07 where
          # the original series started!
          # but to be consistent with the length of the structural
          # model-residuals from the vars (which only start in 1963/01)
          # (see below!), we reduce the number of returns to the adequate
          # length already here:
          # this means that we remove the first five return-entries in 
          # sp500_returns via
          sp500_returns <- tail(sp500_returns,  -5)
          # and then, because the series calculated the returns until 2018/02,
          # we remove the last two entries as well:
          sp500_returns <- head(sp500_returns, -2)
          # a quick check shows us that now sp500_returns has exactly 
          # 660 entries (ranging from 1963/01 until 2017/12):
...
#######################
    ...
    for(k in 1:500000){
          ...
          # all data in SVAR.data starts in 1960-07-01 and runs until
          # xx; the data.frame
          # has 690 rows;
          ...
          # When trying tuore-run this script, we saw that because we get
          # the data for industrial production (ip) from the data.frame 
          # additional_vars and it only starts in July 1962, we had to 
          # also make SVAR.data.sub start in July 1962;
          # hence, contrary to the above, also when re-running this script,
          # the residuals in epsilon_t have 660 rows (14 less then before);
          # and because we lose 6 data points due to the 6 lags in the VAR-model,
          # the residuals in epsilon_t start only in 1963-01-01!
          ...
          # Marcel, 03.03.2020: Note that the original 
          # data-series runs only until 2017/12; this is why we have 
          # limited the length of dates below to 2017/12!
          dates = seq(from = as.Date("1963-01-01"), 
                      to = as.Date("2017-12-01"), by = 'month')
             ...
             dplyr::select(financial_h1)) >= k1){
              ...
              if(any(epsilon_t %>% filter(yearmon >= "Dec 2007" &
                 ...
                 if(sum(epsilon_t %>% filter(yearmon >= "Dec 2007" &
                       ...
                       # the data.frame 'sp500_returns which we have prepared above
                       # and which contains the variable 'returns' 
                       ...
                       # we fit an AR(1)-model to sp500_merge_vxo$mvol:
                       ar_1 <- arima(as.ts(sp500_returns), order=c(1, 0, 0))
...
#######################
    ...
    ## only calculate 60 psi-matrices!)
    #----------------------------------------------
    ...
    # n is the number of variable sin the VAR-system (in our case n = 3!)
    
    calculate_MA_matrices <- function(list, k, n){
      if(k == 0){
        return(diag(n))
      }else if(k == 1){
        return(
          calculate_MA_matrices(list, k-1, n)%*%list[[1]]
        )
      }else if(k == 2){
        return(
          calculate_MA_matrices(list, k-1, n)%*%list[[1]] + 
          calculate_MA_matrices(list, k-2, n)%*%list[[2]]
        )
      }else if(k == 3){
        return(
          calculate_MA_matrices(list, k-1, n)%*%list[[1]] + 
          calculate_MA_matrices(list, k-2, n)%*%list[[2]] + 
          calculate_MA_matrices(list, k-3, n)%*%list[[3]]
        )
      }else if (k == 4){
        return(
          calculate_MA_matrices(list, k-1, n)%*%list[[1]] + 
          calculate_MA_matrices(list, k-2, n)%*%list[[2]] + 
          calculate_MA_matrices(list, k-3, n)%*%list[[3]] + 
          calculate_MA_matrices(list, k-4, n)%*%list[[4]]
        )
      }else if(k == 5){
        return(
          calculate_MA_matrices(list, k-1, n)%*%list[[1]] + 
          ...
          calculate_MA_matrices(list, k-3, n)%*%list[[3]] + 
          calculate_MA_matrices(list, k-4, n)%*%list[[4]] + 
          calculate_MA_matrices(list, k-5, n)%*%list[[5]]
        )
      }else if(k >=6){
        ...
        return(
          ...
          calculate_MA_matrices(list, k-4, n)%*%list[[4]] + 
          calculate_MA_matrices(list, k-5, n)%*%list[[5]] + 
          calculate_MA_matrices(list, k-6, n)%*%list[[6]]
        )
      }
    }
    ...
    for(i in 0:25){
      psi_list[[length(psi_list)+1]] <- calculate_MA_matrices(B_list, i, 3)
      # and we rename the entry to know to which
      # iteration it belongs
