...
#######################
    ...
    for(k in 1:10000){
          ...
          if(epsilon_t %>% dplyr::filter(yearmon == "Oct 1987") %>%
             dplyr::select(financial_h1) >= k1){
              ...
              # Marcel, 06.05.2020: Interestingly, after going throw the
              # code of Ludvigson et al (2018) step by step and having derived
              # the structural errors from the same A_0 (impact matrix) to be sure
              # that our method and Ludvigson et al's (2018) method for the 
              # calculation of the structural errors is identical,
              # the sign difference is obviously not the case anymore; therefore
              # I conclude that it was an artefact of the particular cases 
              # we had compared on 04.05.2020 above or the result of having
              # mixed up the two columns macro_h1 and financial_h1;
              # most probably, the reason was the latter!
              ...
              if(any(epsilon_t %>% filter(yearmon >= "Dec 2007" &
                     ...
                     # Marcel, 06.05.2020: Note that the sign-difference was obviously 
                     # due to me mixing up the columns macro_h1 and financial_h1 and
                     # accordingly naming them wrongly when creating the matrix 
                     # of structural errors in epsilon_t;
                     # after correcting this issue, the alleged problem with the sign
                     # difference from above is not the case anymore!
                 ...
                 if(sum(epsilon_t %>% filter(yearmon >= "Dec 2007" &
                       ...
                       if((lambda1 - c_M) >= 0 & (lambda1 - c_F) >= 0){
                         ...
                         # We have mentioned this issue already above and should 
                         # also point this out in the thesis-text itself!
