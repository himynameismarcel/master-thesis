...
### corresponds to VAR8 mentioned in the main text of the thesis, but instead of
### HP-detrending all variables like Bloom does in his original contribution,
### we here follow Jurado et al. (2015) that replicate Bloom's estimations
### but without detrending the respective varibales!
...
for (i in seq_along(cols_log)) {
  var8_bloom_JFV[cols_log[i]] <- log(var8_bloom_JFV[cols_log[i]])
}
...
## This one data-frame will then we fed into a ggplot using 'facets' to
...
ggsave(file="var8_plot_all_NoHP_until2008.pdf", width = 210, height = 297, units = "mm")
